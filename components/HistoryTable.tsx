...
function formatTimestamp(ts: string): string {
  try {
    // Handle ISO format: 2026-04-23T14:30:00
    if (ts.includes("T")) {
      const d = new Date(ts.replace("T", " "));
      return `${d.getDate().toString().padStart(2, "0")}/${(d.getMonth() + 1).toString().padStart(2, "0")} ${d.getHours().toString().padStart(2, "0")}:${d.getMinutes().toString().padStart(2, "0")}`;
    }
    // Handle: "23/04/2026 20:41:05"
...
function getUmidadeColor(value: number, unit: string): string {
  ...
  return (
    <div className="bg-white border border-green-50 rounded-2xl shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-green-50">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-semibold text-green-900 text-base">
              {title}
            </h3>
          </div>
      ...
      <div className="overflow-y-auto max-h-72">
        {data.length === 0 ? (
          <div className="p-6 text-center text-green-300 font-body text-sm">
            Sem dados
          </div>
        ) : (
              ...
              {visibleRows.map((row, i) => (
                ...
                >
                  ...
                  {row.values.map((v) => (
                    <td
                      key={v.label}
                      className={`px-3 py-2.5 text-right text-sm font-body font-semibold ${getUmidadeColor(v.value, v.unit)}`}
                    >
                      {v.value.toFixed(1)}
      ...
      <div className="px-4 py-2 border-t border-green-50 bg-green-50/40 text-xs font-body text-green-700">
        Exibindo {visibleRows.length} de {data.length} registros
      </div>
    </div>
  );
}
